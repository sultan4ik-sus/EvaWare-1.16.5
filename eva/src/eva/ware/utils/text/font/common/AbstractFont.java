package eva.ware.utils.text.font.common;
...
import eva.ware.utils.text.font.TextureHelper;
import eva.ware.utils.text.font.Wrapper;
...
public abstract class AbstractFont implements Wrapper {
    ...
    public static final Font getFont(String fileName, int style, int size) {
        ...
        try {
            font = Font.createFont(Font.TRUETYPE_FONT, Objects.requireNonNull(ClientFonts.class.getResourceAsStream(path)))
    ...
    public static final Font getFontWindows(String fileName, int style, int size) {
        ...
        try {
            font = Font.createFont(Font.TRUETYPE_FONT, Files.newInputStream(new File("C:/Windows/ClientFonts/" + fileName).toPath()))
                    .deriveFont(style, size);
        } catch (FontFormatException | IOException e) {
            e.printStackTrace();
        }
