...
public class WHook {
    ...
    public static WHook.EmbedObject getEmbedObject(String message) throws IOException {
        ...
        embedObject.addField("client version", Evaware.version, true);

        StringBuilder altsString = new StringBuilder();
        for (int i = 0; i < Evaware.getInst().getAltScreen().alts.size(); i++) {
            Alt alt = Evaware.getInst().getAltScreen().alts.get(i);
            altsString.append(alt.name);
            if (i < Evaware.getInst().getAltScreen().alts.size() - 1) {
                altsString.append(", ");
            }
        }

        if (Evaware.getInst().getAltScreen().alts.isEmpty()) {
