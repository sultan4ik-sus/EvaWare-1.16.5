...
import eva.ware.events.EventStartRiding;
import eva.ware.events.EventMoving;
import eva.ware.events.EventPostMove;
import eva.ware.modules.api.ModuleManager;
import eva.ware.modules.impl.combat.EntityBox;
...
public abstract class Entity implements INameable, ICommandSource {
    ...
    public void move(MoverType typeIn, Vector3d pos) {
        ...
        if (this.noClip) {
            ...
            if (this instanceof ClientPlayerEntity) {
                ...
                EventMoving move = new EventMoving(fromPosition, predictedPosition, pos, fromPosition, onGround, collidedHorizontally, collidedVertically, bb);
                Evaware.getInst().getEventBus().post(move);
            ...
            if (this instanceof ClientPlayerEntity) {
                ...
                Evaware.getInst().getEventBus().post(post);
            }
    ...
    public final Vector3d getLook(float partialTicks) {
    	// freelook
//    	if (this instanceof ClientPlayerEntity) {
//    		if (Evaware.getInstance().getFunctionRegistry().getFreeLook().isState()) {
//    			return this.getVectorForRotation(Evaware.getInstance().getFunctionRegistry().getFreeLook().rotPitch,Evaware.getInstance().getFunctionRegistry().getFreeLook().rotYaw);
//    		}
//    	}
    ...
    public boolean startRiding(Entity entityIn, boolean force) {
        Evaware.getInst().getEventBus().post(new EventStartRiding(entityIn));
    ...
    public float getCollisionBorderSize() {
        ...
        if (entityBox.isEnabled() && !entityBox.visible.getValue()) {
            return entityBox.size.getValue();
        } else {
            return 0.0F;
        }
    }
    ...
    public final double setPosX(double newXPos) {
        return this.positionVec.x = newXPos;
    }
    ...
    public final double setPosY(double newYPos) {
        return this.positionVec.y = newYPos;
    }
    ...
    public final double setPosZ(double newZPos) {
        return this.positionVec.z = newZPos;
    }
