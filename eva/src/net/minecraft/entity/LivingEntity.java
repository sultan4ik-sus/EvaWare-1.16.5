...
import eva.ware.events.EventDamageReceive;
import eva.ware.events.EventJump;
import eva.ware.modules.api.ModuleManager;
import eva.ware.modules.impl.combat.AutoExplosion;
import eva.ware.modules.impl.combat.CrystalAura;
import eva.ware.modules.impl.combat.HitAura;
import eva.ware.modules.impl.misc.AntiPush;
import eva.ware.modules.impl.movement.Scaffold;
import eva.ware.modules.impl.visual.ViewModel;
...
public abstract class LivingEntity extends Entity {
    ...
    private static final AttributeModifier SPRINTING_SPEED_BOOST = new AttributeModifier(SPRINTING_SPEED_BOOST_ID,
            "Sprinting speed boost", (double) 0.3F, AttributeModifier.Operation.MULTIPLY_TOTAL);
    protected static final DataParameter<Byte> LIVING_FLAGS = EntityDataManager.createKey(LivingEntity.class,
            DataSerializers.BYTE);
    private static final DataParameter<Float> HEALTH = EntityDataManager.createKey(LivingEntity.class,
            DataSerializers.FLOAT);
    private static final DataParameter<Integer> POTION_EFFECTS = EntityDataManager.createKey(LivingEntity.class,
            DataSerializers.VARINT);
    private static final DataParameter<Boolean> HIDE_PARTICLES = EntityDataManager.createKey(LivingEntity.class,
            DataSerializers.BOOLEAN);
    private static final DataParameter<Integer> ARROW_COUNT_IN_ENTITY = EntityDataManager.createKey(LivingEntity.class,
            DataSerializers.VARINT);
    private static final DataParameter<Integer> BEE_STING_COUNT = EntityDataManager.createKey(LivingEntity.class,
            DataSerializers.VARINT);
    private static final DataParameter<Optional<BlockPos>> BED_POSITION = EntityDataManager
            .createKey(LivingEntity.class, DataSerializers.OPTIONAL_BLOCK_POS);
    ...
    protected LivingEntity(EntityType<? extends LivingEntity> type, World worldIn) {
        ...
        this.brain = this.createBrain(new Dynamic<>(nbtdynamicops, nbtdynamicops
                .createMap(ImmutableMap.of(nbtdynamicops.createString("memories"), nbtdynamicops.emptyMap()))));
    }
    ...
    public static AttributeModifierMap.MutableAttribute registerAttributes() {
        return AttributeModifierMap.createMutableAttribute().createMutableAttribute(Attributes.MAX_HEALTH)
                .createMutableAttribute(Attributes.KNOCKBACK_RESISTANCE)
                .createMutableAttribute(Attributes.MOVEMENT_SPEED).createMutableAttribute(Attributes.ARMOR)
    ...
    protected void updateFallState(double y, boolean onGroundIn, BlockState state, BlockPos pos) {
        ...
        if (!this.world.isRemote && this.fallDistance > 3.0F && onGroundIn) {
            ...
            if (!state.isAir()) {
                ...
                int i = (int) (150.0D * d0);
                ((ServerWorld) this.world).spawnParticle(new BlockParticleData(ParticleTypes.BLOCK, state),
                        this.getPosX(), this.getPosY(), this.getPosZ(), i, 0.0D, 0.0D, 0.0D, (double) 0.15F);
            }
        }
    ...
    public void baseTick() {
        ...
        if (this.isAlive()) {
            if (this.isEntityInsideOpaqueBlock()) {
                ...
                double d0 = this.world.getWorldBorder().getClosestDistance(this)
        ...
        if (this.isAlive()) {
            if (this.areEyesInFluid(FluidTags.WATER)
                    && !this.world.getBlockState(new BlockPos(this.getPosX(), this.getPosYEye(), this.getPosZ()))
                    .isIn(Blocks.BUBBLE_COLUMN)) {
                if (!this.canBreatheUnderwater() && !EffectUtils.canBreatheUnderwater(this) && !flag1) {
                    ...
                    if (this.getAir() == -20) {
                        ...
                        for (int i = 0; i < 8; ++i) {
                            ...
                            this.world.addParticle(ParticleTypes.BUBBLE, this.getPosX() + d2, this.getPosY() + d3,
                                    this.getPosZ() + d4, vector3d.x, vector3d.y, vector3d.z);
                        }
                ...
                if (!this.world.isRemote && this.isPassenger() && this.getRidingEntity() != null
                        && !this.getRidingEntity().canBeRiddenInWater()) {
                    this.stopRiding();
                }
    ...
    public boolean getMovementSpeed() {
        return this.ticksExisted % 5 == 0 && this.getMotion().x != 0.0D && this.getMotion().z != 0.0D
                && !this.isSpectator() && EnchantmentHelper.hasSoulSpeed(this) && this.func_230296_cM_();
    }

    protected void addSprintingEffect() {
        Vector3d vector3d = this.getMotion();
        this.world.addParticle(ParticleTypes.SOUL,
                this.getPosX() + (this.rand.nextDouble() - 0.5D) * (double) this.getWidth(), this.getPosY() + 0.1D,
                this.getPosZ() + (this.rand.nextDouble() - 0.5D) * (double) this.getWidth(), vector3d.x * -0.2D, 0.1D,
                vector3d.z * -0.2D);
    ...
    protected float getSpeedFactor() {
        return this.func_230296_cM_() && EnchantmentHelper.getMaxEnchantmentLevel(Enchantments.SOUL_SPEED, this) > 0
                ? 1.0F
                : super.getSpeedFactor();
    }
        ...
        if (!this.getStateBelow().isAir()) {
            ...
            if (i > 0 && this.func_230296_cM_()) {
                ...
                modifiableattributeinstance
                        .applyNonPersistentModifier(new AttributeModifier(SOUL_SPEED_BOOT_ID, "Soul speed boost",
                                (double) (0.03F * (1.0F + (float) i * 0.35F)), AttributeModifier.Operation.ADDITION));
     ...
     * should retaliate. Is not guaranteed to
     * change our actual active target (for example if we are currently busy
    ...
    public void readAdditional(CompoundNBT compound) {
        ...
        if (compound.contains("Team", 8)) {
            ...
            boolean flag = scoreplayerteam != null
        ...
        if (compound.contains("SleepingX", 99) && compound.contains("SleepingY", 99)
                && compound.contains("SleepingZ", 99)) {
            BlockPos blockpos = new BlockPos(compound.getInt("SleepingX"), compound.getInt("SleepingY"),
    ...
    protected void updatePotionEffects() {
        ...
        if (i > 0) {
            ...
            if (flag && i > 0) {
                ...
                this.world.addParticle(flag1 ? ParticleTypes.AMBIENT_ENTITY_EFFECT : ParticleTypes.ENTITY_EFFECT,
                        this.getPosXRandom(0.5D), this.getPosYRandom(), this.getPosZRandom(0.5D), d0, d1, d2);
            }
        }
    }
    ...
    /**
     * Returns true if all of the potion effects in the specified collection are
     * ambient.
     */
    ...
    protected void onFinishedPotionEffect(EffectInstance effect) {
        ...
        if (!this.world.isRemote) {
            effect.getPotion().removeAttributesModifiersFromEntity(this, this.getAttributeManager(),
                    effect.getAmplifier());
        }
    }
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        if (this.isInvulnerableTo(source)) {
            ...
            if ((source == DamageSource.ANVIL || source == DamageSource.FALLING_BLOCK)
                    && !this.getItemStackFromSlot(EquipmentSlotType.HEAD).isEmpty()) {
                this.getItemStackFromSlot(EquipmentSlotType.HEAD).damageItem(
                        (int) (amount * 4.0F + this.rand.nextFloat() * amount * 2.0F), this, (p_233653_0_) -> {
            ...
            if (flag1) {
                ...
                if (entity1 != null) {
                    ...
                    for (d0 = entity1.getPosZ() - this.getPosZ(); d1 * d1
                            + d0 * d0 < 1.0E-4D; d0 = (Math.random() - Math.random()) * 0.01D) {
                        d1 = (Math.random() - Math.random()) * 0.01D;
                    }

                    this.attackedAtYaw = (float) (MathHelper.atan2(d0, d1) * (double) (180F / (float) Math.PI)
            ...
            if (entity1 instanceof ServerPlayerEntity) {
                CriteriaTriggers.PLAYER_HURT_ENTITY.trigger((ServerPlayerEntity) entity1, this, source, f, amount,
                        flag);
            }
    ...
    private void renderBrokenItemStack(ItemStack stack) {
        if (!stack.isEmpty()) {
            if (!this.isSilent()) {
                this.world.playSound(this.getPosX(), this.getPosY(), this.getPosZ(), SoundEvents.ENTITY_ITEM_BREAK,
                        this.getSoundCategory(), 0.8F, 0.8F + this.world.rand.nextFloat() * 0.4F, false);
            }
    ...
    protected void createWitherRose(@Nullable LivingEntity entitySource) {
        if (!this.world.isRemote) {
            ...
            if (entitySource instanceof WitherEntity) {
                ...
                if (!flag) {
                    ItemEntity itementity = new ItemEntity(this.world, this.getPosX(), this.getPosY(), this.getPosZ(),
        ...
        if (!this.world.isRemote && (this.isPlayer() || this.recentlyHit > 0 && this.canDropLoot()
            ...
            while (i > 0) {
                ...
                this.world.addEntity(
                        new ExperienceOrbEntity(this.world, this.getPosX(), this.getPosY(), this.getPosZ(), j));
            }
        }
    }
    ...
    protected LootContext.Builder getLootContextBuilder(boolean attackedRecently, DamageSource damageSourceIn) {
        LootContext.Builder lootcontext$builder = (new LootContext.Builder((ServerWorld) this.world))
                .withRandom(this.rand).withParameter(LootParameters.THIS_ENTITY, this)
                .withParameter(LootParameters.field_237457_g_, this.getPositionVec())
                .withParameter(LootParameters.DAMAGE_SOURCE, damageSourceIn)
                .withNullableParameter(LootParameters.KILLER_ENTITY, damageSourceIn.getTrueSource())
                .withNullableParameter(LootParameters.DIRECT_KILLER_ENTITY, damageSourceIn.getImmediateSource());
    ...
    public void applyKnockback(float strength, double ratioX, double ratioZ) {
        ...
        if (!(strength <= 0.0F)) {
            ...
            this.setMotion(vector3d.x / 2.0D - vector3d1.x,
                    this.onGround ? Math.min(0.4D, vector3d.y / 2.0D + (double) strength) : vector3d.y,
    ...
    private boolean canGoThroughtTrapDoorOnLadder(BlockPos pos, BlockState state) {
        if (state.get(TrapDoorBlock.OPEN)) {
            ...
            if (blockstate.isIn(Blocks.LADDER)
                    && blockstate.get(LadderBlock.FACING) == state.get(TrapDoorBlock.HORIZONTAL_FACING)) {
                return true;
            }
        }
    ...
    protected float applyArmorCalculations(DamageSource source, float damage) {
        if (!source.isUnblockable()) {
            ...
            damage = CombatRules.getDamageAfterAbsorb(damage, (float) this.getTotalArmorValue(),
            ...
            if (this.isPotionActive(Effects.RESISTANCE) && source != DamageSource.OUT_OF_WORLD) {
                ...
                if (f2 > 0.0F && f2 < 3.4028235E37F) {
                    if (this instanceof ServerPlayerEntity) {
                        ...
                        ((ServerPlayerEntity) source.getTrueSource()).addStat(Stats.DAMAGE_DEALT_RESISTED,
                                Math.round(f2 * 10.0F));
                    }
                }
            }
    ...
    protected void damageEntity(DamageSource damageSrc, float damageAmount) {
        if (!this.isInvulnerableTo(damageSrc)) {
            ...
            if (f > 0.0F && f < 3.4028235E37F && damageSrc.getTrueSource() instanceof ServerPlayerEntity) {
                ((ServerPlayerEntity) damageSrc.getTrueSource()).addStat(Stats.DAMAGE_DEALT_ABSORBED,
                        Math.round(f * 10.0F));
            }
    ...
    /**
     * Returns the maximum health of the entity (what it is able to regenerate up
     * to, what it spawned with, etc)
     */
    ...
    private int getArmSwingAnimationEnd() {
        ...
        ViewModel animation = moduleManager.getViewModel();

        if (animation.isEnabled() && animation.swingAnim.getValue() && this instanceof ClientPlayerEntity) {
            return 25 - (animation.swingSpeed.getValue().intValue() * 2);
        }
    ...
    public void handleStatusUpdate(byte id) {
        switch (id) {
            ...
            case 44:
                ...
                if (flag1) {
                    this.playSound(SoundEvents.ENCHANT_THORNS_HIT, this.getSoundVolume(),
                ...
                if (soundevent1 != null) {
                    this.playSound(soundevent1, this.getSoundVolume(),
                            (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
                }
            ...
            case 46:
                ...
                for (int j = 0; j < 128; ++j) {
                    ...
                    double d1 = MathHelper.lerp(d0, this.prevPosX, this.getPosX())
                            + (this.rand.nextDouble() - 0.5D) * (double) this.getWidth() * 2.0D;
                    double d2 = MathHelper.lerp(d0, this.prevPosY, this.getPosY())
                            + this.rand.nextDouble() * (double) this.getHeight();
                    double d3 = MathHelper.lerp(d0, this.prevPosZ, this.getPosZ())
                            + (this.rand.nextDouble() - 0.5D) * (double) this.getWidth() * 2.0D;
    ...
    public boolean func_233634_a_(Predicate<Item> p_233634_1_) {
        return p_233634_1_.test(this.getHeldItemMainhand().getItem())
                || p_233634_1_.test(this.getHeldItemOffhand().getItem());
    }
    ...
    private void func_233628_a_(Entity p_233628_1_) {
        ...
        if (!p_233628_1_.removed
                && !this.world.getBlockState(p_233628_1_.getPosition()).getBlock().isIn(BlockTags.PORTALS)) {
            vector3d = p_233628_1_.func_230268_c_(this);
        } else {
            vector3d = new Vector3d(p_233628_1_.getPosX(), p_233628_1_.getPosY() + (double) p_233628_1_.getHeight(),
                    p_233628_1_.getPosZ());
        }
    ...
    private final EventJump eventJump = new EventJump();
    ...
    protected void jump() {
        ...
        if (this instanceof ClientPlayerEntity entity) {
            Evaware.getInst().getEventBus().post(eventJump);
        }
        ...
        if (this.isSprinting()) {
            float f1 = (this.rotationYawOffset == Integer.MIN_VALUE ? this.rotationYaw : this.rotationYawOffset) * ((float) Math.PI / 180F);
    ...
    public void jump(float force, float boost) {
        ...
        if (this.isSprinting()) {
            float f1 = (this.rotationYawOffset == Integer.MIN_VALUE ? this.rotationYaw : this.rotationYawOffset)
                    * ((float) Math.PI / 180F);
            this.setMotion(this.getMotion().add(-MathHelper.sin(f1) * boost, 0.0D,
                    (double) (MathHelper.cos(f1) * boost)));
        }
        this.isAirBorne = true;
    }
    ...
    public void travel(Vector3d travelVector) {
        if (this.isServerWorld() || this.canPassengerSteer()) {
            ...
            if (this.isInWater() && this.func_241208_cS_() && !this.func_230285_a_(fluidstate.getFluid())) {
                ...
                if (this.collidedHorizontally && this.isOffsetPositionInLiquid(vector3d2.x,
                        vector3d2.y + (double) 0.6F - this.getPosY() + d8, vector3d2.z)) {
                ...
                if (this.collidedHorizontally && this.isOffsetPositionInLiquid(vector3d4.x,
                        vector3d4.y + (double) 0.6F - this.getPosY() + d7, vector3d4.z)) {
                ...
                AutoExplosion autoExplosion = Evaware.getInst().getModuleManager().getAutoExplosion();
                CrystalAura crystalAura = Evaware.getInst().getModuleManager().getCrystalAura();
                Scaffold scaffold = Evaware.getInst().getModuleManager().getScaffold();
                boolean hitAuraIsNull = hitAura.getTarget() == null || !hitAura.isEnabled();

                // hitAura
                if (hitAura.isEnabled() && hitAura.getOptions().is("Коррекция движения").getValue() && this instanceof ClientPlayerEntity) {
                    vector3d1 = this.getVectorForRotation(hitAura.rotate.y, hitAura.rotate.x);
                }
                float f = this.rotationPitch * ((float) Math.PI / 180F);
                if (hitAura.isEnabled() && hitAura.getOptions().is("Коррекция движения").getValue() && this instanceof ClientPlayerEntity) {
                    f = hitAura.rotate.y * ((float) Math.PI / 180F);
                }

                // autoExplosion
                if (autoExplosion.check() && autoExplosion.isEnabled() && hitAuraIsNull && autoExplosion.options.is("Коррекция движения").getValue() && this instanceof ClientPlayerEntity) {
                    vector3d1 = this.getVectorForRotation(autoExplosion.rotationVector.y, autoExplosion.rotationVector.x);
                }

                // scaffold
                if (scaffold.check() && scaffold.isEnabled() && hitAuraIsNull && this instanceof ClientPlayerEntity) {
                    vector3d1 = this.getVectorForRotation(scaffold.rotation.y, -scaffold.rotation.x);
                }

                // crystalAura
                if (crystalAura.check() && crystalAura.isEnabled() && hitAuraIsNull && crystalAura.options.is("Коррекция движения").getValue() && this instanceof ClientPlayerEntity) {
                    vector3d1 = this.getVectorForRotation(autoExplosion.rotationVector.y, autoExplosion.rotationVector.x);
                }
                ...
                if (d1 > 0.0D) {
                    vector3d = vector3d.add((vector3d1.x / d1 * d3 - vector3d.x) * 0.1D, 0.0D,
                            (vector3d1.z / d1 * d3 - vector3d.z) * 0.1D);
                }
                ...
                if (this.isPotionActive(Effects.LEVITATION)) {
                    d2 += (0.05D * (double) (this.getActivePotionEffect(Effects.LEVITATION).getAmplifier() + 1)
                            - vector3d5.y) * 0.2D;
                    this.fallDistance = 0.0F;
    ...
    public Vector3d func_233626_a_(double p_233626_1_, boolean p_233626_3_, Vector3d p_233626_4_) {
        if (!this.hasNoGravity() && !this.isSprinting()) {
            ...
            if (p_233626_3_ && Math.abs(p_233626_4_.y - 0.005D) >= 0.003D
                    && Math.abs(p_233626_4_.y - p_233626_1_ / 16.0D) < 0.003D) {
                d0 = -0.003D;
            } else {
    ...
    private Vector3d handleOnClimbable(Vector3d p_213362_1_) {
        if (this.isOnLadder()) {
            ...
            if (d2 < 0.0D && !this.getBlockState().isIn(Blocks.SCAFFOLDING) && this.hasStoppedClimbing()
    ...
    private float getRelevantMoveFactor(float p_213335_1_) {
        return this.onGround ? this.getAIMoveSpeed() * (0.21600002F / (p_213335_1_ * p_213335_1_ * p_213335_1_))
    ...
    private void func_241342_a_(Map<EquipmentSlotType, ItemStack> p_241342_1_) {
        ...
        if (itemstack != null && itemstack1 != null
                && ItemStack.areItemStacksEqual(itemstack, this.getItemInHand(EquipmentSlotType.OFFHAND))
                && ItemStack.areItemStacksEqual(itemstack1, this.getItemInHand(EquipmentSlotType.MAINHAND))) {
            ((ServerWorld) this.world).getChunkProvider().sendToAllTracking(this,
                    new SEntityStatusPacket(this, (byte) 55));
    ...
    private void func_241344_b_(Map<EquipmentSlotType, ItemStack> p_241344_1_) {
        ...
        ((ServerWorld) this.world).getChunkProvider().sendToAllTracking(this,
                new SEntityEquipmentPacket(this.getEntityId(), list));
    }

    public ItemStack getArmorInSlot(EquipmentSlotType slot) {
    ...
    public void triggerItemPickupTrigger(ItemEntity item) {
        PlayerEntity playerentity = item.getThrowerId() != null ? this.world.getPlayerByUuid(item.getThrowerId())
                : null;

        if (playerentity instanceof ServerPlayerEntity) {
            CriteriaTriggers.THROWN_ITEM_PICKED_UP_BY_ENTITY.test((ServerPlayerEntity) playerentity, item.getItem(),
                    this);
        }
    }
    ...
    public void onItemPickup(Entity entityIn, int quantity) {
        if (!entityIn.removed && !this.world.isRemote && (entityIn instanceof ItemEntity
                || entityIn instanceof AbstractArrowEntity || entityIn instanceof ExperienceOrbEntity)) {
            ((ServerWorld) this.world).getChunkProvider().sendToAllTracking(entityIn,
                    new SCollectItemPacket(entityIn.getEntityId(), this.getEntityId(), quantity));
        }
    }
    ...
    public boolean canEntityBeSeen(Entity entityIn) {
        ...
        return this.world.rayTraceBlocks(new RayTraceContext(vector3d, vector3d1, RayTraceContext.BlockMode.COLLIDER,
    ...
    public Vector3d getPositon(float partialTicks) {
        return MathUtility.interpolate(getPositionVec(), new Vector3d(lastTickPosX, lastTickPosY, lastTickPosZ), partialTicks);
    }
    ...
    public float getYaw(float partialTicks) {
        return partialTicks == 1.0F ? this.rotationYawHead
                : MathHelper.lerp(partialTicks, this.prevRotationYawHead, this.rotationYawHead);
    }
    ...
    public boolean canBePushed() {
        ...
        if (antiPush.isEnabled() && antiPush.modes.is("Игроки").getValue() && this instanceof ClientPlayerEntity) {
            return false;
        }
        ...
        if (this.isHandActive()) {
            if (ItemStack.areItemsEqualIgnoreDurability(this.getHeldItem(this.getActiveHand()), this.activeItemStack)) {
                ...
                if (--this.activeItemStackUseCount == 0 && !this.world.isRemote
                        && !this.activeItemStack.isCrossbowStack()) {
    ...
    protected void triggerItemUseEffects(ItemStack stack, int count) {
        if (!stack.isEmpty() && this.isHandActive()) {
            ...
            if (stack.getUseAction() == UseAction.EAT) {
                ...
                this.playSound(this.getEatSound(stack), 0.5F + 0.5F * (float) this.rand.nextInt(2),
                        (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
            }
        }
    }
    ...
    private void addItemParticles(ItemStack stack, int count) {
        for (int i = 0; i < count; ++i) {
            Vector3d vector3d = new Vector3d(((double) this.rand.nextFloat() - 0.5D) * 0.1D,
            ...
            this.world.addParticle(new ItemParticleData(ParticleTypes.ITEM, stack), vector3d1.x, vector3d1.y,
    ...
    public AxisAlignedBB getPoseAABB(Pose pose) {
        ...
        return new AxisAlignedBB((double) (-entitysize.width / 2.0F), 0.0D, (double) (-entitysize.width / 2.0F),
                (double) (entitysize.width / 2.0F), (double) entitysize.height, (double) (entitysize.width / 2.0F));
    }
    ...
    private void setSleepingPosition(BlockPos p_213370_1_) {
        this.setPosition((double) p_213370_1_.getX() + 0.5D, (double) p_213370_1_.getY() + 0.6875D,
                (double) p_213370_1_.getZ() + 0.5D);
    }
    ...
    public ItemStack onFoodEaten(World p_213357_1_, ItemStack p_213357_2_) {
        if (p_213357_2_.isFood()) {
            p_213357_1_.playSound((PlayerEntity) null, this.getPosX(), this.getPosY(), this.getPosZ(),
                    this.getEatSound(p_213357_2_), SoundCategory.NEUTRAL, 1.0F,
                    1.0F + (p_213357_1_.rand.nextFloat() - p_213357_1_.rand.nextFloat()) * 0.4F);
    ...
    private void applyFoodEffects(ItemStack p_213349_1_, World p_213349_2_, LivingEntity p_213349_3_) {
        ...
        if (item.isFood()) {
            for (Pair<EffectInstance, Float> pair : item.getFood().getEffects()) {
                if (!p_213349_2_.isRemote && pair.getFirst() != null
                        && p_213349_2_.rand.nextFloat() < pair.getSecond()) {
    ...
    public boolean isBlocking() {
        return this.isHandActive()
                && this.activeItemStack.getItem().getUseAction(this.activeItemStack) == UseAction.BLOCK;
    }
