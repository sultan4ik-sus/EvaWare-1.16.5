        ...
        if (this.isAttachedToEntity())
        {
            ...
            if (this.boostedEntity != null) {
                if (this.boostedEntity.isElytraFlying()) {
                    ...
                    if (Evaware.getInst().getModuleManager().getElytraBooster().isEnabled()) {
                        boostedSpeed = Evaware.getInst().getModuleManager().getElytraBooster().boosterSpeed;
                    }

                    HitAura hitAura = Evaware.getInst().getModuleManager().getHitAura();
                    if (hitAura.isEnabled() && hitAura.getTarget() != null && hitAura.getOptions().is("Коррекция движения").getValue() && boostedEntity instanceof ClientPlayerEntity) {
                        Vector3d vector3d = this.getVectorForRotation(hitAura.rotate.y, hitAura.rotate.x);
