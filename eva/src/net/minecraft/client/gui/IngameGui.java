...
public class IngameGui extends AbstractGui {
    ...
    public TimerUtility watch = new TimerUtility();
    ...
    EventRender2D eventRender2D = new EventRender2D(new MatrixStack(), 1);

    public void renderIngameGui(MatrixStack matrixStack, float partialTicks) {
        eventRender2D.setMatrixStack(matrixStack);
        eventRender2D.setPartialTicks(partialTicks);

        mc.gameRenderer.setupOverlayRendering(2);
        eventRender2D.setType(EventRender2D.Type.PRE);
        if (eventRender2D.getType() == EventRender2D.Type.PRE) {
            Evaware.getInst().getEventBus().post(eventRender2D);
        }
        mc.gameRenderer.setupOverlayRendering();


        mc.gameRenderer.setupOverlayRendering(2);
        eventRender2D.setType(EventRender2D.Type.HIGH);
        if (eventRender2D.getType() == EventRender2D.Type.HIGH) {
            Evaware.getInst().getEventBus().post(eventRender2D);
        }
        mc.gameRenderer.setupOverlayRendering();
        ...
        // Kawase.Render.draw(matrixStack);
        // DisplayUtils.drawWhite();
        ...
        if (!this.mc.gameSettings.hideGUI) {
            ...
            Evaware.getInst().getEventBus().post(eventCancelOverlay);
            ...
            if (SelfDestruct.unhooked) {
                ...
                this.overlayPlayerList.getAnimation().update();
                this.overlayPlayerList.setVisible(this.mc.gameSettings.keyBindPlayerList.isKeyDown() && (!this.mc.isIntegratedServerRunning() || this.mc.player.connection.getPlayerInfoMap().size() > 1 || scoreobjective1 != null));
                if (this.overlayPlayerList.isVisible() || this.overlayPlayerList.getAnimation().getValue() != -400.0) {
                    this.scalePre();
                    this.overlayPlayerList.func_238523_a_(matrixStack, ClientUtility.getMouse(this.mc.getMainWindow().getScaledWidth(), 0).getX(), scoreboard, scoreobjective1);
                    this.scalePost();
                }
            }
        }
        ...
        mc.gameRenderer.setupOverlayRendering(2);
        eventRender2D.setType(EventRender2D.Type.POST);
    ...
    protected void renderHotbar(float partialTicks, MatrixStack matrixStack) {
        ...
        if (playerentity != null) {
            ...
            this.blit(matrixStack, i - 91 - 1 + playerentity.inventory.currentItem * 20, this.scaledHeight - 22 - 1, 0,
                    22, 24, 22);
        ...
        if (this.remainingHighlightTicks > 0 && !this.highlightingItemStack.isEmpty()) {
            ...
            if (l > 0) {
                ...
                if (Reflector.IForgeItem_getFontRenderer.exists()) {
                    fontrenderer = (FontRenderer) Reflector.call(this.highlightingItemStack.getItem(),
                            Reflector.IForgeItem_getFontRenderer, this.highlightingItemStack);
                }

                if (fontrenderer != null) {
                    i = (this.scaledWidth - fontrenderer.getStringPropertyWidth(itextcomponent)) / 2;
                    fontrenderer.func_238422_b_(p_238453_1_, itextcomponent.func_241878_f(), (float) j, (float) k,
                            16777215 + (l << 24));
                } else {
                    this.getFontRenderer().func_243246_a(p_238453_1_, itextcomponent, (float) j, (float) k,
                            16777215 + (l << 24));
                }
