    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks)
    {
        if (SelfDestruct.unhooked) {
            ...
            if (f >= 1.0F) {
                ...
                int l = MathHelper.ceil((1.0F - MathHelper.clamp(f - 1.0F, 0.0F, 1.0F)) * 255.0F);
                RenderUtility.drawRoundedRect(0, 0, width, height, 0, ColorUtility.rgba(20, 20, 20, l));
                f2 = 1.0F - MathHelper.clamp(f - 1.0F, 0.0F, 1.0F);
            ...
            RenderUtility.drawRoundedRect(0, 0, width, height, 0, ColorUtility.rgba(20, 20, 20, (int) (255 * f2)));
            RenderUtility.drawShadow(width / 2 - ClientFonts.interRegular[24].getWidth("Евачка любимая >_<") / 2 - 2, height / 2 - 4, ClientFonts.interRegular[24].getWidth("Евачка любимая >_<") + 4, ClientFonts.interRegular[24].getFontHeight() + 4, 12, ColorUtility.reAlphaInt(-1, (int) ((20 * f2) * MathHelper.clamp(animation.getOutput(), 0, 1))));
            ClientFonts.interRegular[24].drawCenteredString(matrixStack, "Евачка любимая >_<", width / 2, height / 2, ColorUtility.reAlphaInt(-1, (int) ((255 * f2) * MathHelper.clamp(animation.getOutput(), 0, 1))));
            int l1 = (int) ((double) this.mc.getMainWindow().getScaledHeight() * 0.8325D);
            float f3 = this.asyncReloader.estimateExecutionSpeed();
            this.progress = MathHelper.clamp(this.progress * 0.95F + f3 * 0.050000012F, 0.0F, 1.0F);
            Reflector.ClientModLoader_renderProgressText.call();
