...
import eva.ware.events.EventTick;
...
public class Minecraft extends RecursiveEventLoop<Runnable> implements ISnooperInfo, IWindowEventListener {
    ...
    public Minecraft(GameConfiguration gameConfig) {
        ...
        ModuleManager moduleManager = eva.ware.Evaware.getInst().getModuleManager();
        SelfDestruct selfDestruct = moduleManager.getSelfDestruct();


        if (s != null) {
            this.displayGuiScreen(new ConnectingScreen(selfDestruct.unhooked ? new MainMenuScreen() : new MainScreen(), this, s, i));
        } else {
            this.displayGuiScreen(selfDestruct.unhooked ? new MainMenuScreen() : new MainScreen());
        }
    ...
    public void setDefaultMinecraftTitle() {
        ...
        if (selfDestruct.unhooked) {
            ...
            this.mainWindow.setWindowTitle(Evaware.name + " #" + Evaware.build);
        }
    }
    ...
    public String getVersion() {
        ...
        return selfDestruct.unhooked ? "Optifine 1.16.5" : this.launchedVersion;
    }
    ...
    public void displayGuiScreen(@Nullable Screen guiScreenIn) {
        ...
        if (guiScreenIn == null && this.world == null) {
            guiScreenIn = selfDestruct.unhooked ? new MainMenuScreen() : new MainScreen();
    ...
    private void tick(boolean isDebug, @Nullable LongTickDetector detector) {
        eva.ware.Evaware.getInst().getEventBus().post(eventTick);
            ...
            if (!this.player.isRowingBoat()) {
                ...
                for (Hand hand : Hand.values()) {
                    ...
                    if (this.objectMouseOver != null) {
                        switch (this.objectMouseOver.getType()) {
                            case ENTITY -> {
                                ...
                                boolean hitAuraIsStateAndTargetNotNull = hitAura.isEnabled() && hitAura.getTarget() != null;
                    ...
                    if (!itemstack.isEmpty()) {
                        ...
                        if (itemCooldown.isEnabled() && itemEnum != null && itemCooldown.isCurrentItem(itemEnum) && player.getCooldownTracker().hasCooldown(itemEnum.getItem())) {
    ...
    public void runTick() {
        ...
        if (this.world != null) {
            eva.ware.Evaware.getInst().getEventBus().post(player.tick);
    ...
    public void loadWorld(ClientWorld worldClientIn) {
        ...
        eva.ware.Evaware.getInst().getEventBus().post(new EventChangeWorld());
    }
    ...
    public boolean isEntityGlowing(Entity entity) {
        return entity.isGlowing() || this.player != null && this.player.isSpectator() && this.gameSettings.keyBindSpectatorOutlines.isKeyDown() && entity.getType() == EntityType.PLAYER && !Evaware.getInst().getModuleManager().getNoRender().isEnabled() && !Evaware.getInst().getModuleManager().getNoRender().element.is("Эффект свечения").getValue();
    }
