...
public class FirstPersonRenderer {
    ...
    private void transformFirstPerson(MatrixStack matrixStackIn, HandSide handIn, float swingProgress) {
        ...
        ViewModel viewModel = moduleManager.getViewModel();


        if (viewModel.isEnabled() && viewModel.swingAnim.getValue()) {
            int i = handIn == HandSide.RIGHT ? 1 : -1;
            float power = viewModel.swingPower.getValue() * 10;
    ...
    private void transformSideFirstPerson(MatrixStack matrixStackIn, HandSide handIn, float equippedProg) {
        ...
        int i = handIn == HandSide.RIGHT ? 1 : -1;
        if (viewModel.onlyAura.getValue() && hitAura.getTarget() == null && viewModel.isEnabled() && viewModel.swingAnim.getValue()) {
            matrixStackIn.translate((float) i * 0.56F, -0.52F + equippedProg * (-0.6F), -0.72F);
            return;
        }
        matrixStackIn.translate((float) i * 0.56F, -0.52F + equippedProg * ((viewModel.isEnabled() && viewModel.swingAnim.getValue()) ? 0.0F : -0.6F), -0.72F);
    }
        ...
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(handIn)) {
            ...
            if (stack.isEmpty()) {
                ...
                if (viewModel.isEnabled()) {
                    if (flag3) {
                        matrixStackIn.translate(viewModel.x.getValue(),
                                viewModel.y.getValue(), viewModel.z.getValue());
                    } else {
                        matrixStackIn.translate(-viewModel.x.getValue(),
                                viewModel.y.getValue(), viewModel.z.getValue());
                    }
                ...
                if (player.isHandActive() && player.getItemInUseCount() > 0 && player.getActiveHand() == handIn) {
                    ...
                    ViewModel swingAnimation = moduleManager.getViewModel();
                    ...
                    if (!swingAnimation.isEnabled() || !swingAnimation.swingAnim.getValue())
                        matrixStackIn.translate((double) ((float) l * f6), (double) f7, (double) f10);

                    this.transformSideFirstPerson(matrixStackIn, handside, equippedProgress);

                    if (swingAnimation.isEnabled() && swingAnimation.swingAnim.getValue() && handIn.equals(Hand.MAIN_HAND) && (!itemStackMainHand.getItem().equals(Items.SHIELD))) {
