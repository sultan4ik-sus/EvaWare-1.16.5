...
//    }
    ...
    private double calcCameraDistance(double startingDistance) {
        EventCancelOverlay eventCancelOverlay = new EventCancelOverlay(EventCancelOverlay.Overlays.CAMERA_CLIP);
        ...
        boolean isBetter = betterMinecraft.isEnabled() && betterMinecraft.smoothCamera.getValue();
         for (int i = 0; i < 8; ++i) {
             ...
             if(!eventCancelOverlay.isCancel() && isBetter) {
             ...
             if  (!SelfDestruct.unhooked && isBetter) {
                 interpolatedDistance = MathUtility.fast((float) interpolatedDistance, (float) startingDistance, 10);
