...
import eva.ware.modules.impl.combat.NoEntityTrace;
...
public class GameRenderer implements IResourceManagerReloadListener, AutoCloseable {
    ...
    public double getFOVModifier(ActiveRenderInfo activeRenderInfoIn, float partialTicks, boolean useFOVSetting) {
        if (this.debugView) {
            ...
            boolean isBetter = betterMinecraft.isEnabled() && betterMinecraft.smoothCamera.getValue();
    ...
    private void hurtCameraEffect(MatrixStack matrixStackIn, float partialTicks) {
        ...
        if (!Evaware.getInst().getModuleManager().getSelfDestruct().unhooked) {
    ...
    public void renderHand(MatrixStack p_renderHand_1_, ActiveRenderInfo p_renderHand_2_, float p_renderHand_3_,
        ...
        ModuleManager moduleManager = Evaware.getInst().getModuleManager();
        ViewModel glassHand = moduleManager.getViewModel();
            ...
            if (this.mc.loadingGui != null) {
                ...
                try {
                    if (Reflector.ForgeHooksClient_drawScreen.exists()) {
                        Reflector.callVoid(Reflector.ForgeHooksClient_drawScreen, this.mc.currentScreen, matrixstack, i,
                                j, this.mc.getTickLength());
                    } else {
