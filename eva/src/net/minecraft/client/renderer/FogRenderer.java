...
public class FogRenderer
{
    ...
    public static void updateFogColor(ActiveRenderInfo activeRenderInfoIn, float partialTicks, ClientWorld worldIn, int renderDistanceChunks, float bossColorModifier)
    {
        ...
        else
        {
            ...
            if (!worldTweaks.isEnabled()) {
                red = (float) vector3d4.getX();
                green = (float) vector3d4.getY();
                blue = (float) vector3d4.getZ();
            } else if (worldTweaks.isEnabled() && !SelfDestruct.unhooked) {
                if (worldTweaks.options.is("Кастомный туман").getValue()) {
                    final float[] colors = ColorUtility.rgba(worldTweaks.mode.is("Клиент") ? Theme.rectColor : worldTweaks.colorFog.getValue());
                    red = colors[0];
                    green = colors[1];
                    blue = colors[2];
                }
    ...
    public static void setupFog(ActiveRenderInfo p_setupFog_0_, FogType p_setupFog_1_, float p_setupFog_2_, boolean p_setupFog_3_, float p_setupFog_4_)
    {
        ...
        else
        {
            ...
            if (!SelfDestruct.unhooked) {
                if (worldTweaks.isEnabled() && worldTweaks.options.is("Своя дистанция тумана").getValue()) {
                    f3 = worldTweaks.fogDistance.getValue();
                    f4 = f3 * 100;
                }
            }
