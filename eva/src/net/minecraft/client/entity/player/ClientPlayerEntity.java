...
import eva.ware.command.interfaces.CommandDispatcher;
...
import eva.ware.modules.impl.movement.AutoSprint;
...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
    ...
    public void tick() {
        if (this.world.isBlockLoaded(new BlockPos(this.getPosX(), 0.0D, this.getPosZ()))) {
            if (!Evaware.getInst().playerOnServer) {
                Evaware.getInst().playerOnServer = true;
            }

            Evaware.getInst().getEventBus().post(eventUpdate);
            super.tick();
    ...
    private void onUpdateWalkingPlayer() {
        ...
        Evaware.getInst().getEventBus().post(ea);
        ...
        if (this.isCurrentViewEntity()) {
            ...
            Evaware.getInst().getEventBus().post(eventMotion);
        ...
        Evaware.getInst().getEventBus().post(new EventPostMotion());
    ...
    public void sendChatMessage(String message) {
        ...
        CommandDispatcher commandDispatcher = Evaware.getInst().getCommandDispatcher();
        ...
        if (Evaware.getInst().getModuleManager().getChatHelper().emoji.getValue()
                && !message.startsWith("/") && !message.startsWith(".")
                && Evaware.getInst().getModuleManager().getChatHelper().isEnabled()) {
            String[] emojiArray = {"(UwU)", "(>w<)", "(O_O)", " (T_T)", "(-_-)", "(._.)", "(YwY)", "(.-.)"};
            Random rand = new Random();
            int randomIndex = rand.nextInt(emojiArray.length);
            message = message + " " + emojiArray[randomIndex];
        }
    ...
    public void closeScreen() {
        ...
        Evaware.getInst().getEventBus().post(event);
        if (!event.isCancel()) {
    ...
    private boolean shouldBlockPushPlayer(BlockPos pos) {
        ...
        if (antiPush.isEnabled() && antiPush.modes.is("Блоки").getValue()) {
            return false;
        }
    ...
    public void setSprinting(boolean sprinting) {
        Evaware.getInst().getEventBus().post(eventSprint);
    ...
    public void livingTick() {
        ...
        if (this.isHandActive() && !this.isPassenger()) {
            Evaware.getInst().getEventBus().post(eventNoSlow);
    ...
    public static class WindowClickMemory {
        ...
        public TimerUtility timerWaitAction = new TimerUtility();
