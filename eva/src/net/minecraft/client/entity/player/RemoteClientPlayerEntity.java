...
public class RemoteClientPlayerEntity extends AbstractClientPlayerEntity
{
	...
	public void resolve() {
		this.backUpX = this.getPosX();
		this.backUpY = this.getPosY();
		this.backUpZ = this.getPosZ();
		Vector3d position = Minecraft.getInstance().player.getPositionVec();
		Vector3d target;
		Vector3d from = new Vector3d(this.prevServerX, this.prevServerY, this.prevServerZ);
		Vector3d to = new Vector3d(this.serverX, this.serverY, this.serverZ);
