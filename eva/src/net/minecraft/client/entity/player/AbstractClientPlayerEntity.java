...
public abstract class AbstractClientPlayerEntity extends PlayerEntity
{
    ...
    public ResourceLocation getLocationSkin() {
        WorldTweaks worldTweaks = Evaware.getInst().getModuleManager().getWorldTweaks();
        NetworkPlayerInfo networkplayerinfo = this.getPlayerInfo();
        return worldTweaks.updateSkin(networkplayerinfo == null ? DefaultPlayerSkin.getDefaultSkin(this.getUniqueID()) : networkplayerinfo.getLocationSkin(), this);
    }

    @Nullable
    public ResourceLocation getLocationCape() {
        ModuleManager moduleManager = Evaware.getInst().getModuleManager();
        if (this instanceof ClientPlayerEntity && !SelfDestruct.unhooked && (!moduleManager.getWorldTweaks().options.is("Маленький игрок").getValue() && moduleManager.getWorldTweaks().isEnabled()) && !(moduleManager.getNoRender().isEnabled() && moduleManager.getNoRender().element.is("Плащ").getValue())) {
            GifUtility gifUtility = new GifUtility();
            int i = gifUtility.getFrame(35, 100, true);
            return new ResourceLocation("eva/images/gif/cape/cape-" + i + ".png");
        }
