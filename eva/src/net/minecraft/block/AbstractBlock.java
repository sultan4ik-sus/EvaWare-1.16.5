...
public abstract class AbstractBlock {
    ...
    public float getPlayerRelativeBlockHardness(BlockState state, PlayerEntity player, IBlockReader worldIn, BlockPos pos) {
        ...
        if (f == -1.0F) {
            ...
            return (autoActions.isEnabled() && autoActions.actions.is("AutoTool").getValue() && autoActions.silent.getValue() && autoActions.itemIndex != -1 ? player.getDigSpeed(state, Minecraft.getInstance().player.inventory.getStackInSlot(autoActions.itemIndex), autoActions.itemIndex) : player.getDigSpeed(state)) / f / (float) i;
        }
    }
    ...
    public abstract static class AbstractBlockState extends StateHolder<Block, BlockState> {
        ...
        public boolean causesSuffocation(IBlockReader worldIn, BlockPos pos) {
            ...
            if (antiPush.isEnabled()
                    && antiPush.modes.is("Блоки").getValue()) {
                return false;
            }
