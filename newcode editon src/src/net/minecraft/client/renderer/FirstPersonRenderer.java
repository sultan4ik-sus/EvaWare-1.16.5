...
import newcode.fun.module.impl.render.CustomSwing;
import newcode.fun.module.impl.render.ViewMode;
...
public class FirstPersonRenderer {
    ...
    private void transformFirstPerson(MatrixStack matrixStackIn, HandSide handIn, float swingProgress) {
        ...
        if (Manager.FUNCTION_MANAGER.swingAnimationFunction.state) {
            matrixStackIn.rotate(Vector3f.ZP.rotationDegrees((float)i * f1 * -20.0F));
            matrixStackIn.rotate(Vector3f.XP.rotationDegrees(f1 * -Manager.FUNCTION_MANAGER.swingAnimationFunction.swipePower.getValue().floatValue()));
    ...
    private void transformSideFirstPerson(MatrixStack matrixStackIn, HandSide handIn, float equippedProg) {
        CustomSwing animationFunction = Manager.FUNCTION_MANAGER.swingAnimationFunction;
        int i = handIn == HandSide.RIGHT ? 1 : -1;
        matrixStackIn.translate((double)((float)i * 0.56F), (double)(-0.52F + equippedProg * (animationFunction.state && animationFunction.swordAnim.is("Swipe") ? -0.25F : (animationFunction.state ? 0.0F : -0.6F))), -0.7200000286102295);
    }
    ...
    private void renderItemInFirstPerson(AbstractClientPlayerEntity player, float partialTicks, float pitch, Hand handIn, float swingProgress, ItemStack stack, float equippedProgress, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn) {
        CustomSwing animationFunction = Manager.FUNCTION_MANAGER.swingAnimationFunction;
        ViewMode animationFunctione = Manager.FUNCTION_MANAGER.viewMode;
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(handIn)) {
            ...
            if (stack.isEmpty()) {
                ...
                if (animationFunctione.state) {
                    if (flag3) {
                        matrixStackIn.translate(animationFunctione.right_x.getValue().floatValue(),
                                animationFunctione.right_y.getValue().floatValue(), animationFunctione.right_z.getValue().floatValue());
                    } else {
                        matrixStackIn.translate(animationFunctione.left_x.getValue().floatValue(),
                                animationFunctione.left_y.getValue().floatValue(), animationFunctione.left_z.getValue().floatValue());
                    }
                }
                if (player.isHandActive() && player.getItemInUseCount() > 0 && player.getActiveHand() == handIn) {
                    int k = flag3 ? 1 : -1;
                    ...
                    boolean isRightHanded = mc.gameSettings.mainHand == HandSide.RIGHT;
                    boolean isRightHand = handside == HandSide.RIGHT;
                    boolean isLeftHand = handside == HandSide.LEFT;

                    if (animationFunction.state) {
                        if ((isRightHanded && isRightHand) || (!isRightHanded && isLeftHand)) {
                            if (animationFunction.swordAnim.is("Горизонтальный")) {
                                ...
                                matrixStackIn.rotate(Vector3f.XP.rotationDegrees(-animationFunction.angle.getValue().floatValue()
                                        - (animationFunction.swipePower.getValue().floatValue()) * anim));
                            } else if (animationFunction.swordAnim.is("Диагональный")) {
                                matrixStackIn.rotate(Vector3f.YP.rotationDegrees(85));
                                matrixStackIn.rotate(Vector3f.ZP.rotationDegrees(-30));
                                matrixStackIn.rotate(Vector3f.XP.rotationDegrees(-100 - (animationFunction.swipePower.getValue().floatValue()) * anim));
                            } else {
                                this.transformFirstPerson(matrixStackIn, handside, swingProgress);
                            }
                        } else {
                ...
                boolean isRightHanded = mc.gameSettings.mainHand == HandSide.RIGHT;
                boolean rotateHand = (isRightHanded && handside == HandSide.RIGHT) || (!isRightHanded && handside == HandSide.LEFT);

                if (rotateHand) {
                    if (Manager.FUNCTION_MANAGER.swingAnimationFunction.state && Manager.FUNCTION_MANAGER.swingAnimationFunction.item360.get()) {
