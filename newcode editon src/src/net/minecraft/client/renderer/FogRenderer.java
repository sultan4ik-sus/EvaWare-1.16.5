...
import newcode.fun.module.impl.render.Ambience;
...
public class FogRenderer {
    ...
    public static void updateFogColor(ActiveRenderInfo activeRenderInfoIn, float partialTicks, ClientWorld worldIn, int renderDistanceChunks, float bossColorModifier) {
        ...
        Ambience customWorld = Manager.FUNCTION_MANAGER.ambience;
        ...
        if (fluidstate.isTagged(FluidTags.WATER)) {
            ...
            if (customWorld.state && customWorld.fog.get()) {
                final float[] colors = RenderUtils.IntColor.rgb((Manager.FUNCTION_MANAGER.hud2.mode.is("13") ? ColorUtils.getColorStyle(0.0F) : StyleManager.getCurrentStyle().getSecondaryColor()));
                red = colors[0];
                green = colors[1];
                blue = colors[2];
            } else {
    ...
    public static void setupFog(ActiveRenderInfo p_setupFog_0_, FogType p_setupFog_1_, float p_setupFog_2_, boolean p_setupFog_3_, float p_setupFog_4_) {
        ...
        if (f >= 0.0F) {
            ...
            if (fluidstate.isTagged(FluidTags.LAVA)) {
                ...
                if (customWorld.state && customWorld.fog.get()) {
                    f3 /= customWorld.distanceFog.getValue().floatValue();
                } else {
