...
public abstract class LivingRenderer<T extends LivingEntity, M extends EntityModel<T>> extends EntityRenderer<T> implements IEntityRenderer<T, M> {
    ...
    public void renderFixed(T entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entityIn, this, partialTicks, matrixStackIn, bufferIn, packedLightIn)) {
            ...
            float f7 = entityIn == Minecraft.getInstance().getRenderViewEntity() ?
                    MathHelper.lerp(partialTicks, entityIn.prevRotationPitchHead, entityIn.rotationPitchHead)
            ...
            RenderType rendertype = this.func_230496_a_(entityIn, flag1, true, false);
            ...
            if (rendertype != null) {
                ...
                float[] colors = new float[]{1, 1, 1, flag2 ? 0.15F : 1.0F};

                if ((entityIn.hurtTime > 0 || entityIn.deathTime > 0) && Manager.FUNCTION_MANAGER.hitColor.state) {
                    colors = RenderUtils.IntColor.rgb(ColorUtils.interpolateColor(-1, ColorUtils.getColorStyle(5), Manager.FUNCTION_MANAGER.hitColor.intensivity.getValue().floatValue()));
                    colors[3] = 1F;
                    i = 655360;
                }

                this.entityModel.render(matrixStackIn, ivertexbuilder, packedLightIn, i, colors[0],colors[1],colors[2],colors[3]);
            }
    ...
    public void render(T entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entityIn, this, partialTicks, matrixStackIn, bufferIn, packedLightIn)) {
            ...
            if (rendertype != null) {
                ...
                this.entityModel.render(matrixStackIn, ivertexbuilder, packedLightIn, i, 1.0F, 1.0F, 1.0F, flag2 ? (Manager.FUNCTION_MANAGER.seeInvisibles.isState() ? (flag2 && entityIn instanceof PlayerEntity ? Manager.FUNCTION_MANAGER.seeInvisibles.alpha.getValue().floatValue() : 0) : 0F) : 1.0F);
            }
