...
import newcode.fun.utils.glu.Project;
...
public class GameRenderer implements IResourceManagerReloadListener, AutoCloseable {
    ...
    public void getMouseOver(float partialTicks) {
        ...
        if (entity != null && this.mc.world != null) {
            ...
            double distance = (double) this.mc.playerController.getBlockReachDistance();
            this.mc.objectMouseOver = entity.pick(distance, partialTicks, false);
    ...
    public double getFOVModifier(ActiveRenderInfo activeRenderInfoIn, float partialTicks, boolean useFOVSetting) {
        if (this.debugView) {
            ...
            factor = AnimationMath.fast(factor, flag ? 4 : 1, 5);
            d0 /= factor;
    ...
    public void hurtCameraEffect(MatrixStack matrixStackIn, float partialTicks) {
    ...
    public void applyBobbing(MatrixStack matrixStackIn, float partialTicks) {
    ...
    public Matrix4f getProjectionMatrix(ActiveRenderInfo activeRenderInfoIn, float partialTicks, boolean useFovSetting) {
        ...
        if (this.cameraZoom != 1.0F) {
            matrixstack.translate((double)this.cameraYaw, (double)(-this.cameraPitch), 0.0);
            matrixstack.scale(this.cameraZoom, this.cameraZoom, 1.0F);
        }
        matrixstack.getLast().getMatrix().mul(Matrix4f.perspective(this.getFOVModifier(activeRenderInfoIn, partialTicks, useFovSetting), Manager.FUNCTION_MANAGER.aspect.isState() ? 1 / ((Float) Manager.FUNCTION_MANAGER.aspect.widthe.getValue() + 0.265f): (float)this.mc.getMainWindow().getFramebufferWidth() / (float)this.mc.getMainWindow().getFramebufferHeight(), 0.05F, this.clipDistance));
        return matrixstack.getLast().getMatrix();
    }

    public Matrix4f getProjectionMatrix(ActiveRenderInfo activeRenderInfoIn, float partialTicks, boolean useFovSetting, int pass) {
        MatrixStack matrixstack = new MatrixStack();
        matrixstack.getLast().getMatrix().setIdentity();
        ...
        matrixstack.translate((float) (-(pass * 2 - 1)) * 0.07F, 0.0F, 0.0F);
        this.clipDistance = this.farPlaneDistance * 2.0F;
        ...
        matrixstack.translate((float) (pass * 2 - 1) * 0.1F, 0.0F, 0.0F);
        return matrixstack.getLast().getMatrix();
    }
    ...
    public void setupCameraTransform(MatrixStack stack, float partialTicks, int pass, boolean flip) {
        ...
        GlStateManager.translatef((float) (-(pass * 2 - 1)) * 0.07F, 0.0F, 0.0F);
        ...
        Project.gluPerspective((float) this.getFOVModifier(activeRender, partialTicks, true), (float) this.mc.getMainWindow().getWidth() / (float) this.mc.getMainWindow().getHeight(), 0.05F, this.clipDistance);
        GlStateManager.matrixMode(5888);
        GlStateManager.loadIdentity();


        GlStateManager.translatef((float) (pass * 2 - 1) * 0.1F, 0.0F, 0.0F);
        ...
        if (this.mc.gameSettings.viewBobbing)
        {
            if (this.mc.getRenderViewEntity() instanceof PlayerEntity)
            {
                ...
                float f5 = entityplayer.distanceWalkedModified - entityplayer.prevDistanceWalkedModified;
                float f1 = -(entityplayer.distanceWalkedModified + f5 * partialTicks);
        ...
        if (this.mc.gameSettings.getPointOfView().ordinal() > 0) {
            ...
            GlStateManager.rotatef(mc.player.rotationPitch - pitch, 1.0F, 0.0F, 0.0F);
            GlStateManager.rotatef(mc.player.rotationYaw - yaw, 0.0F, 1.0F, 0.0F);

            GlStateManager.rotatef(yaw - mc.player.rotationYaw, 0.0F, 1.0F, 0.0F);
            GlStateManager.rotatef(pitch - mc.player.rotationPitch, 1.0F, 0.0F, 0.0F);

        }


        GlStateManager.rotatef(mc.player.prevRotationPitch + (mc.player.rotationPitch - mc.player.prevRotationPitch) * partialTicks, 1.0F, 0.0F, 0.0F);
        GlStateManager.rotatef(mc.player.prevRotationYaw + (mc.player.rotationYaw - mc.player.prevRotationYaw) * partialTicks + (!flip ? 180.0F : 0), 0.0F, 1.0F, 0.0F);
    ...
    public void displayItemActivation(ItemStack stack) {
        EventOverlaysRender overlaysRender = new EventOverlaysRender(EventOverlaysRender.OverlayType.TOTEM);
