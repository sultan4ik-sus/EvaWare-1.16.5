...
import newcode.fun.control.events.impl.player.*;
...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
    ...
    private void onUpdateWalkingPlayer() {
        ...
        if (!(this.isCurrentViewEntity() && Manager.FUNCTION_MANAGER.freeCam.state)) {
            ...
            Manager.FUNCTION_MANAGER.timer.updateTimer(eventPre.getYaw(), eventPre.getPitch(), eventPre.getX(), this.getPosY(), eventPre.getZ());
    ...
    public void sendChatMessage(String message) {
        ...
        if (Manager.COMMAND_MANAGER.isMessage) {
            return;
        }
    ...
    private boolean shouldBlockPushPlayer(BlockPos pos) {
        if (Manager.FUNCTION_MANAGER.noPushFunction.state && Manager.FUNCTION_MANAGER.noPushFunction.modes.get(1))
            return false;
    ...
    public boolean isInBlock() {
        ...
        // ѕровер€ем, €вл€етс€ ли игрок внутри блока по оси Y
        // »грок находитс€ в блоках, если его Y-координата не имеет дробной части (он не в воздухе)
        return posY % 1 != 0;
    }
    ...
    public static class WindowClickMemory {
        ...
        public TimerUtil timerWaitAction = new TimerUtil();
