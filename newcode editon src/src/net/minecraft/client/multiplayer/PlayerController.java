...
import newcode.fun.control.events.impl.game.EventDestroyBlock;
import newcode.fun.control.events.impl.player.EventWindowClick;
import newcode.fun.control.Manager;
import newcode.fun.module.impl.player.NoInteract;
...
public class PlayerController implements IPlayerControllerMP {
    ...
    public boolean clickBlock(BlockPos loc, Direction face) {
        if (this.mc.player.blockActionRestricted(this.mc.world, loc, this.currentGameType)) {
            ...
            if (this.currentGameType.isCreative()) {
                ...
                EventManager.call(new EventDestroyBlock());
    ...
    public ActionResultType processRightClickBlock(ClientPlayerEntity player, ClientWorld worldIn, Hand hand, BlockRayTraceResult blockRayTraceResult) {
        NoInteract interactFunction = Manager.FUNCTION_MANAGER.noInteractFunction;
        ...
        if (!this.mc.world.getWorldBorder().contains(blockpos)) {
            ...
            if (this.currentGameType == GameType.SPECTATOR) {
                ...
                boolean flag2 = interactFunction.state && interactFunction.allBlocks.get() || interactFunction.state &&
                        ...
                        ||  Manager.FUNCTION_MANAGER.auraFunction.getTarget() != null;
                if (!flag2) {
                    if (!flag1) {
    ...
    public ItemStack windowClick(final int windowId, final int slotId, final int mouseButton, final ClickType type, final PlayerEntity player) {
        ...
        EventManager.call(new EventWindowClick(EventWindowClick.ClickStage.PRE, windowId, slotId, mouseButton, type));
        this.connection.sendPacket(new CClickWindowPacket(windowId, slotId, mouseButton, type, itemstack, short1));
        EventManager.call(new EventWindowClick(EventWindowClick.ClickStage.POST, windowId, slotId, mouseButton, type));
        return itemstack;
    }
