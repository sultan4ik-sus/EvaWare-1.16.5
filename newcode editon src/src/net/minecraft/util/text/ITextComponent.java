...
public interface ITextComponent extends Message, ITextProperties {
    ...
    default <T> Optional<T> func_230534_b_(IStyledTextAcceptor<T> acceptor, Style style) {
        if (Manager.FUNCTION_MANAGER != null && Manager.FUNCTION_MANAGER.nameProtect.state) {
            return acceptor.accept(style, Manager.FUNCTION_MANAGER.nameProtect.patch(this.getUnformattedComponentText()));
        }
    ...
    default <T> Optional<T> func_230533_b_(ITextAcceptor<T> acceptor) {
        if (Manager.FUNCTION_MANAGER != null && Manager.FUNCTION_MANAGER.nameProtect.state) {
            return acceptor.accept(Manager.FUNCTION_MANAGER.nameProtect.patch(this.getUnformattedComponentText()));
        }
