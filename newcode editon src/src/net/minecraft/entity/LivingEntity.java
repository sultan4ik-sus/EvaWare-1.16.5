...
import newcode.fun.control.events.impl.player.EventStrafe;
...
public abstract class LivingEntity extends Entity {
    ...
    private int getArmSwingAnimationEnd() {
        CustomSwing swingAnimationFunction = (Manager.FUNCTION_MANAGER.swingAnimationFunction);
    ...
    protected void jump() {
        ...
        if (Manager.FUNCTION_MANAGER.auraFunction.state && Manager.FUNCTION_MANAGER.auraFunction.settings.get(1)) {
            if (this.isSprinting()) {
                float f1 = Manager.FUNCTION_MANAGER.auraFunction.rotate.x * ((float) Math.PI / 180F);
...
/*        if (Managment.FUNCTION_MANAGER.auraFunction.state && Managment.FUNCTION_MANAGER.auraFunction.correction.get()) {
        ...
        if (Manager.FUNCTION_MANAGER.autoExplosionFunction.check()) {
            if (this.isSprinting()) {
                float f1 = Manager.FUNCTION_MANAGER.autoExplosionFunction.server.x * ((float) Math.PI / 180F);
                this.setMotion(this.getMotion().add((double) (-MathHelper.sin(f1) * 0.2F), 0.0D, (double) (MathHelper.cos(f1) * 0.2F)));
            }
...
//        }
    ...
    public void travel(Vector3d travelVector) {
        if (this.isServerWorld() || this.canPassengerSteer()) {
            ...
            if (this.isInWater() && this.func_241208_cS_() && !this.func_230285_a_(fluidstate.getFluid())) {
                ...
                if (Manager.FUNCTION_MANAGER.auraFunction.state && Manager.FUNCTION_MANAGER.auraFunction.settings.get(1) && this instanceof ClientPlayerEntity) {
                    vector3d1 = this.getVectorForRotation(Manager.FUNCTION_MANAGER.auraFunction.rotate.y, Manager.FUNCTION_MANAGER.auraFunction.rotate.x);
                }
                float f = this.rotationPitch * ((float) Math.PI / 180F);
                if (Manager.FUNCTION_MANAGER.auraFunction.state && Manager.FUNCTION_MANAGER.auraFunction.settings.get(1) && this instanceof ClientPlayerEntity) {
                    f = Manager.FUNCTION_MANAGER.auraFunction.rotate.y * ((float) Math.PI / 180F);
                }
    ...
    public boolean canBePushed() {
        if (Manager.FUNCTION_MANAGER.noPushFunction.state && Manager.FUNCTION_MANAGER.noPushFunction.modes.get(0) &&
    ...
    public Vector3d getPositon(float partialTicks) {
        return MathUtil.interpolate2(this.getPositionVec(), new Vector3d(this.lastTickPosX, this.lastTickPosY, this.lastTickPosZ), partialTicks);
    }
    public boolean isUsingItem() {
        return this.isHandActive() && !this.getActiveItemStack().isEmpty();
    }
}
