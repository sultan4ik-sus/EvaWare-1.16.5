...
public abstract class AbstractArrowEntity extends ProjectileEntity {
        ...
        if (this.inGround && !flag) {
            ...
            while (!this.removed) {
                ...
                if (raytraceresult != null && !flag) {
                    if (entityraytraceresult != null && raytraceresult.getType() == RayTraceResult.Type.ENTITY && entityraytraceresult.getEntity() instanceof ClientPlayerEntity) {
                        EventManager.call(new EventDamage(EventDamage.DamageType.ARROW));
                    }
    ...
    public void setShooter(@Nullable Entity entityIn) {
        ...
        if (entityIn instanceof PlayerEntity) {
            this.pickupStatus = ((PlayerEntity) entityIn).abilities.isCreativeMode ? PickupStatus.CREATIVE_ONLY : PickupStatus.ALLOWED;
        }
    }
    ...
    public void onCollideWithPlayer(PlayerEntity entityIn) {
        if (!this.world.isRemote && (this.inGround || this.getNoClip()) && this.arrowShake <= 0) {
            boolean flag = this.pickupStatus == PickupStatus.ALLOWED || this.pickupStatus == PickupStatus.CREATIVE_ONLY && entityIn.abilities.isCreativeMode || this.getNoClip() && this.func_234616_v_().getUniqueID() == entityIn.getUniqueID();
