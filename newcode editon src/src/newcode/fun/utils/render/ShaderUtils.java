...
import newcode.fun.utils.misc.FileUtil;
...
public class ShaderUtils implements IMinecraft {
    ...
    public ShaderUtils(String fragmentShaderLoc) {
        ...
        try {
            ...
            int fragmentShaderID = switch (fragmentShaderLoc) {
                ...
                default ->
                        createShader(mc.getResourceManager().getResource(new ResourceLocation("newcode/shader/" + fragmentShaderLoc)).getInputStream(), GL_FRAGMENT_SHADER);
            };
    ...
    public static void init() {
        OUTLINE_SHADER = new ShaderUtils("outlineShade");
    ...
    public void drawQuads(final float x,
        ...
        RenderUtils.Render2D.quadsBegin(x, y, width, height, GL_QUADS);
    }
    ...
    public static void drawQuads() {
        ...
        GL11.glVertex2f(0.0F, (float)Math.max(sr.getScaledHeight(), 1));
        GL11.glTexCoord2f(1.0F, 0.0F);
        GL11.glVertex2f((float)Math.max(sr.getScaledWidth(), 1), (float)Math.max(sr.getScaledHeight(), 1));
        GL11.glTexCoord2f(1.0F, 1.0F);
        GL11.glVertex2f((float)Math.max(sr.getScaledWidth(), 1), 0.0F);
        GL11.glEnd();
    }
    ...
    String outlineShader =
            "#version 330 core\n" +
                    ...
                    "uniform float radius;\n" +
                    "bool isEdge(vec2 uv, vec2 size, float radius) {\n" +
                    "    vec2 margin = vec2(size.x - radius * 2.0, size.y - radius * 2.0);\n" +
                    "    vec2 p = uv - 0.5;\n" +
                    "    bool edge = (abs(p.x) > margin.x || abs(p.y) > margin.y);\n" +
                    "    return edge;\n" +
                    "}\n" +
                    "void main() {\n" +
                    "    vec2 uv = TexCoord * rectSize;\n" +
                    "    uv -= rectPos;\n" +
                    "    if (isEdge(uv, rectSize, radius)) {\n" +
                    "        FragColor = color;\n" +
                    "    } else {\n" +
                    "        discard;\n" +
                    "    }\n" +
